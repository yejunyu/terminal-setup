...
require_file "docs/plans/2026-03-27-best-effort-font-install-design.md"
require_file "docs/plans/2026-03-27-best-effort-font-install.md"
require_file "tests/test_zshrc_uninstall_fallback.sh"
require_file "tests/test_zshrc_brew_shellenv.sh"
require_file "tests/test_best_effort_font_install.sh"
require_file "tests/test_go_proxy_setup.sh"
require_file "tests/test_wsl_layout.sh"
...
bash -n "$ROOT_DIR/tests/test_best_effort_font_install.sh"
bash -n "$ROOT_DIR/tests/test_go_proxy_setup.sh"
bash -n "$ROOT_DIR/tests/test_wsl_layout.sh"
...
grep -q "Maple Mono NF CN" "$ROOT_DIR/configs/wezterm/.wezterm.lua"
grep -q "font-maple-mono-nf-cn" "$ROOT_DIR/setup.sh"
grep -q "font-maple-mono-nf-cn" "$ROOT_DIR/lib/common.sh"
grep -q "configure_nvim_clipboard" "$ROOT_DIR/lib/common.sh"
grep -q 'grep -qF -- "\$marker"' "$ROOT_DIR/lib/common.sh"
grep -q 'vim.opt.clipboard = "unnamedplus"' "$ROOT_DIR/lib/common.sh"
grep -q 'clip.exe' "$ROOT_DIR/lib/common.sh"
grep -q 'pbcopy' "$ROOT_DIR/lib/common.sh"
